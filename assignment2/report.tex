...
solves all that are in $RD^\subseteq(S_2)$ ($reach \models RD^\subseteq(S_2)$.
\end{proof}
...
Now, we consider some properties about the correctness relation.

SHOULD WE PRESENT THE DEFINITION OF THE RELATION HERE?
...
\end{lemma}
\begin{proof}
$tr \sim Y$ means that for all $x \in DOM (tr)$ we have $(x,SRD(tr)(x)) \in Y$. Taking
...
The proof is the same as in the book, but we
