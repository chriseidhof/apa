...
\Var_\star\} \subseteq RD_{entry}(l) \text{ if } l \in init(S_\star) \}\]
...
In order to apply it we need to make sure that function $F_{RD_\star}^S$ (as
defined in chapter 1 of the book) is a monotone function in a complete lattice.
...
For $F_{RD_\star}^S$ we use the function defined in section 1.3, and because of its
