...
The second result simply states that the, ina aolution to the constraints, the analysis results are
...
occorrence of a certain variable in a trace.
...
correct result $Y$ at a certain point, then any ``bigger'' result
will still be correct (altough less optimal). 
...
The follwing result corresponds to Lemma 2.20 in the book. 
It is similar to Lemma \ref{lem218} in the sense that it conncerns what is propagated
...
The proof is the same as in the previous case, taking $S_2$ instead of $S_1$.
...
\begin{cor}[Execution Correctness (cf. Corollary 2.22)]
...
DO WE NEED TO MAKE THE PROOF EXPLICITLY OR IS THIS ENOUGH?
\end{proof}
...
program point (as seen in \ref{cor221}) while the new constraint give us correct results to start with.
...
\end{theo}
\begin{proof}
Let $reach \models RD_\star^\subseteq(S_\star)$. Then, in particualar,
$\{(x,?) \mid x \in \Var_\star\} \subseqeq N(init(S_star))$  is satisfied. So,
$tr_0 \sim init(S_star)$. Also,
$reach \models RD^\subseteq(S_\star)$, thus \ref{cor221} holds. Applying it
to $\pair{S_\star,\sigma,\tr_0}$, knowing that $tr_0 \sim init(S_star),
...
The following result is analogous to Lemma 2.15 in the book and holds wheter we consider undefinedness
...
endomorphism in a complete lattice.
