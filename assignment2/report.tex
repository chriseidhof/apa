...
\begin{align} 
\label{def_constr}
RD^{\subseteq}(S) = & \; \{X(l) \supseteq N(l) \setminus kill_{RD}(B^l)) \cup gen_{RD}(B^l) \mid B^l \in blocks(S)\} \\
\cup & \; \{N(l) \supseteq X(l') \mid (l', l) \in flow(S) \}
\end{align}
...
$(l, l') \in flow(S)$ we have $reach_{entry}(l') \supseteq reach_{exit}(l)$.
\end{lemma}
\begin{proof}
...
$tr \sim X(l)$ implies $tr \sim N(l')$ for all $(l, l')$ in $flow(S)$.
\end{lemma}
\begin{proof}
By \ref{lem218}, $X(l) \subseteq N(l')$. We then apply \ref{lemmasq}.
\end{proof}
